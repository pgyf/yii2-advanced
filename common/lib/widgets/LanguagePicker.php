...
class LanguagePicker extends \yii\base\Widget
{
    ...
    private $_SKINS = [
        self::SKIN_DROPDOWN => [
            'itemTemplate' => '<li><a href="{link}" title="{title}"><i class="{language}"></i> {name}</a></li>',
            'activeItemTemplate' => '<a href="" title="{title}"><i class="{language}"></i> {name}</a>',
            'parentTemplate' => '<div class="language-picker dropdown-list {size}"><div>{activeItem}<ul>{items}</ul></div></div>',
        ],
        self::SKIN_BUTTON => [
            'itemTemplate' => '<a href="{link}" title="{title}"><i class="{language}"></i> {name}</a>',
            'activeItemTemplate' => '<a href="{link}" title="{title}" class="active"><i class="{language}"></i> {name}</a>',
     ...
     */
    private $_SIZES = [
        self::SIZE_SMALL => 'common\lib\assets\languagepicker\LanguageSmallIconsAsset',
        self::SIZE_LARGE => 'common\lib\assets\languagepicker\LanguageLargeIconsAsset',
    ];
    /**
     ...
     */
    public $languagePluginAsset = 'common\lib\assets\languagepicker\LanguagePluginAsset';
    /**
    ...
    public static function widget($config = array())
    {
        if (empty($config['languages']) || !is_array($config['languages'])) {
            ...
            $config['languages'] = array_values(Yii::$app->urlManager->languages);
        }
