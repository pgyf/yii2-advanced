...
class m130524_201442_init extends Migration
{
    public function up()
    {
        ...
        //id字段类型
        $idFieldType = $this->bigInteger()->notNull();
        //创建用户
        $createUserField = [
            'create_user' => $idFieldType,
        ];
        //更新用户
        $updateUserField = [
            'update_user' => $idFieldType,
        ];
        //创建时间
        $createTimeField = [
        ...
        + $createUserField + $updateUserField
       ...
       //用户token表
        $this->createTable('{{%user_token}}', [
            'id' => $this->bigPrimaryKey(),
            'user_id' => $idFieldType,
            'type' => $this->smallInteger()->notNull(),   //用户类型  0验证邮箱 1邮箱找回密码 3验证手机 4手机找回密码
        ...
        $time = time();
        $columns = ['type','username', 'password', 'status', 'create_time', 'update_time', 'auth_key'];
